defmodule AgentJido.Pages.LivebookParser do
  @moduledoc """
  ...
  Category is **not** required in frontmatter — it is derived from the file's
  subdirectory path (docs/, training/, features/, build/, community/).
  ...
  Optional:
  - `category` - Category override atom (normally derived from path)
  ...
  - `track` - Training track atom
  - `difficulty` - Difficulty level atom
  - `duration_minutes` - Estimated duration (integer)
  ...
  alias AgentJido.Pages.ContentExpander
  ...
  def parse(path, contents) do
    contents = ContentExpander.expand(contents)
  ...
  """
  @spec parse_markdown(String.t(), String.t()) :: {map(), String.t()}
  def parse_markdown(path, contents) do
    case Regex.split(@frontmatter_separator, contents, parts: 2) do
      ...
      [_only_body] ->
        raise ArgumentError,
              "Missing frontmatter in page file #{inspect(path)}. " <>
  ...
  """
  @spec parse_livebook(String.t(), String.t()) :: {map(), String.t()}
  ...
  defp validate_types!(attrs, path) do
    validations = [
      ...
      {:prerequisites, &is_list/1, "must be a list"},
