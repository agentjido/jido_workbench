%{
  ...
  order: 1,
  prerequisites: [],
  purpose: "Provide a self-serve map of canonical docs paths and how they connect to Learn, Concepts, Guides, Contributors, Operations, and Reference",
  related: ["docs/getting-started", "docs/concepts/_hub", "docs/guides/_hub", "docs/reference/_hub",
   "docs/learn/_hub", "docs/contributors/_hub", "ecosystem/overview"],
  source_files: ["priv/content_plan/**/*.md"],
  source_modules: ["AgentJido.ContentPlan"],
  prompt_overrides: %{
    document_intent: "Create the entry page for /docs that routes users by intent: learn, build, contribute, troubleshoot, or verify exact API details.",
    ...
    must_include: ["Link to each docs section with a one-line description of what it covers",
     "Intent-based routing: 'I want to learn' → Learn, 'I need to build' → Learn/Guides, 'I need contributor policy' → Contributors, 'Something is broken' → Guides/Operations, 'I need exact details' → Reference"],
    ...
    required_links: ["/docs/getting-started", "/docs/learn", "/docs/concepts", "/docs/guides",
     "/docs/contributors", "/docs/operations", "/docs/reference"],
    min_words: 200,
    max_words: 500,
