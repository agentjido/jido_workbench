defmodule AgentJidoWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AgentJidoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AgentJidoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_admin" do
    test "allows authenticated admin users", %{conn: conn} do
      ...
      assert updated_socket.assigns.current_scope.user.is_admin
    end

    test "redirects authenticated non-admin users", %{conn: conn, user: user} do
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, user_token) |> get_session()
  ...
  describe "require_admin_user/2" do
    ...
    test "does not redirect authenticated admin users", %{conn: conn} do
      admin_user = admin_user_fixture()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AgentJidoWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
