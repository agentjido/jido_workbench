defmodule AgentJidoWeb.ContentOpsGithubLiveTest do
  use AgentJidoWeb.ConnCase, async: false

  import Phoenix.LiveViewTest

  setup do
    original_token = System.get_env("GITHUB_TOKEN")
    System.delete_env("GITHUB_TOKEN")

    on_exit(fn ->
      if original_token do
        System.put_env("GITHUB_TOKEN", original_token)
      else
        System.delete_env("GITHUB_TOKEN")
      end
    end)

    :ok
  end

  test "renders github dashboard on dev route", %{conn: conn} do
    {:ok, _view, html} = live(conn, "/dev/contentops/github")

    assert html =~ "GitHub Issues &amp; PRs"
    assert html =~ "GITHUB_TOKEN environment variable is not set."
  end

  test "blocks mutating events when github mutations are disabled", %{conn: conn} do
    {:ok, view, _html} = live(conn, "/dev/contentops/github")

    html =
      render_hook(view, "solve_issue", %{
        "number" => "10",
        "title" => "Example issue"
      })

    assert html =~ "GitHub mutations are disabled for this environment."

    html =
      render_hook(view, "merge_pr", %{
        "number" => "22",
        "title" => "Example PR"
      })

    assert html =~ "GitHub mutations are disabled for this environment."
  end

  test "public github route is not available", %{conn: conn} do
    conn = get(conn, "/contentops/github")
    assert conn.status == 404
  end
end
