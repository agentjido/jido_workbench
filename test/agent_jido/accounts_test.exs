defmodule AgentJido.AccountsTest do
  use AgentJido.DataCase

  alias AgentJido.Accounts

  import AgentJido.AccountsFixtures
  alias AgentJido.Accounts.{User, UserToken}
  ...
  describe "admin access helpers" do
    test "admin?/1 returns true only for admin users" do
      user = user_fixture()
    ...
    test "promote_user_to_admin/1 updates the admin flag" do
      user = user_fixture()
      refute user.is_admin

      assert {:ok, updated_user} = Accounts.promote_user_to_admin(user)
      assert updated_user.is_admin
    end

    test "promote_user_to_admin_by_email/1 returns not_found for unknown users" do
      assert {:error, :not_found} = Accounts.promote_user_to_admin_by_email("missing@example.com")
    end
  end
