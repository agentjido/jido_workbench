if Mix.env() in [:dev, :test] do
  defmodule Mix.Tasks.Gen.DocJson do
    ...
    @fixed_tvalue_patterns [
      # Match `ChatMember` `status` fixed value.
      ", always “([^”]+)”$",
      # Match `type`/`source` fixed value variants.
      "must be (\\S+)$"
    ]
    ...
    defp fixed_tvalue_re(i) do
      case Enum.at(@fixed_tvalue_patterns, i) do
        nil -> nil
    ...
    @result_type_patterns [
      ...
      "On success, a (\\S+) object is returned",
      "(array of \\S+) .+is returned",
      " (\\S+) is returned",
      "of a (\\S+) object.$",
      "Returns (\\S+) on success",
      "Returns the (\\S+) of",
      "Returns.+as (\\S+) object",
      "Returns.+as a (\\S+) object",
      "Returns.+as (\\S+) on success",
      "Returns a (\\S+) object",
      "returns a (\\S+) object",
    ...
    defp result_type_re(i) do
      case Enum.at(@result_type_patterns, i) do
        nil -> nil
        pattern -> Regex.compile!(pattern)
      end
    end
